...
//  SolarSystemArKit
//
//  Created by Ali Mert Kaya on 6.08.2023.
//
...
class ViewController: UIViewController, ARSCNViewDelegate {
    ...
    override func viewDidLoad() {
        ...
        let world = createSphere(radius: 0.2, content: "world.png", vector: SCNVector3(x: 0, y: 0.2, z: -1))
        let mars = createSphere(radius: 0.1, content: "mars.png", vector: SCNVector3(x: 0.5, y: 0.2, z: -1))
        let venus = createSphere(radius: 0.25, content: "venus.png", vector: SCNVector3(x: 1, y: 0.2, z: -1))
        
        sceneView.scene.rootNode.addChildNode(world)
        sceneView.scene.rootNode.addChildNode(mars)
        sceneView.scene.rootNode.addChildNode(venus)
        // Objeleri 3B göstermeyi kolaylaştırır
